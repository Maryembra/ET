...
public class CarService {
    ...
    public CarResponse saveCar(Car car) throws Exception {
        // Vous pouvez ajouter une logique de validation ici si nécessaire
        Car savedCar = carRepository.save(car);

        // Obtenez le client lié à cette voiture (vous pouvez récupérer le client avec un FeignClient ou un autre service)
        Client client = clientFeignClient.getClientById(car.getClient_id());

        // Retournez la réponse avec les informations du car et du client
        return CarResponse.builder()
                .id(savedCar.getId())
                .brand(savedCar.getBrand())
                .client(client)
                .matricue(savedCar.getMatricule())
